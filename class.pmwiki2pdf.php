...
class pmwiki2pdf {
	...
	private function change_code_html() {
		try {
            ...
            // transform tags unsupported HTML
            if(!empty($this->cfg['unsupported'])) {
                $this->preg_replace_html_unsupported($this->cfg['unsupported']);
            }
    ...
    private function preg_replace_html_unsupported($var) {
        try {

                //while(preg_match($var, $this->html)) {
                    //$pattern = "/<".$var."[\s\S]*<\/".$var.">/";
	...
	private function verif_config() {
		try {
            ...
            // verif config to unsupported tags html
            $this->verif_config_html_unsupported();
    ...
    private function verif_config_html_unsupported() {
        try {

            $this->cfg['unsupported']=""

            if( !empty($this->cfg['skip']) ) {
                if( strstr($this->cfg['skip'], ",") ) {
                    $this->cfg['unsupported'] = explode(",", $this->cfg['skip']);
                }
            }
            unset ($this->cfg['skip']);
